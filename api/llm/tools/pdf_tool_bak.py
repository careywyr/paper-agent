...
def _column_boxes(page, footer_margin=50, header_margin=50, no_image_text=True):
...
def load_pdf(file_path: str) -> list:
    ...
    for page_num in range(len(doc)):
        ...
        for table in tables:
            ...
            try:
                # 创建表格区域的像素图
                pix = page.get_pixmap(matrix=fitz.Matrix(2, 2), clip=table_bbox)
                # 生成图片文件名
                img_dir = os.path.join(os.path.dirname(file_path), "table_images")
                os.makedirs(img_dir, exist_ok=True)
                img_path = os.path.join(img_dir, f"page{page_num}_table{len(table_bboxes)}.png")
                # 保存图片
                pix.save(img_path)
                # 将图片路径添加到表格对象中
                table.image_path = img_path
                print(f"表格已保存为图片: {img_path}")
        ...
        for i, bbox in enumerate(bboxes):
            ...
            print(f"is_table: {is_table}, overlapping_table_bbox: {overlapping_table_bbox}")
            # 如果是表格且这个表格还没有被处理过
...
if __name__ == "__main__":
    filename = "/Users/leafw/Downloads/a.pdf"
    
    # 提取全部内容
    all_contents = load_pdf(filename)
