...
def model_settings(current_llm):
    st.markdown(
        "<h1 style='text-align: center; font-size: 32px;'>模型设置</h1>",
    ...
    if "active_model" not in st.session_state:
        st.session_state.active_model = config_manager.get(
            "active_model",
            (
                "kimi"
                if isinstance(current_llm, KimiLlm)
                else "deepseek"
                if isinstance(current_llm, DeepseekLlm)
                else "ollama"
                if isinstance(current_llm, OllamaLlm)
                else None
            ),
        )
    ...
    def save_model_config():
        ...
        # 清除current_llm缓存，确保下次使用时重新初始化
        if "current_llm" in st.session_state:
            del st.session_state.current_llm
    ...
    def load_model_config():
        ...
        if config:
            ...
            st.session_state.kimi_api_base = kimi_config.get(
            ...
            st.session_state.deepseek_api_base = deepseek_config.get(
                "api_base", "https://api.deepseek.com/v1"
            )
            st.session_state.deepseek_model = deepseek_config.get(
                "model", "deepseek-chat"
            )

            # Ollama配置
            ollama_config = config.get("ollama", {})
            st.session_state.ollama_url = ollama_config.get(
    ...
    with st.expander("🤖 Kimi", expanded=st.session_state.active_model == "kimi"):
        ...
        if kimi_enabled:
            ...
            selected_kimi_model = st.selectbox(
                "选择模型",
                options=kimi_models,
                index=kimi_models.index(
                    st.session_state.get("kimi_model", "moonshot-v1-128k")
                ),
    ...
    # Deepseek设置
    with st.expander(
        "🔍 Deepseek", expanded=st.session_state.active_model == "deepseek"
    ):
        col1, col2 = st.columns([6, 1])
        with col2:
            deepseek_enabled = st.toggle(
                ...
                on_change=lambda: (
                    setattr(
                        ...
                        "active_model",
                        "deepseek"
                        if not st.session_state.active_model == "deepseek"
                        else None,
                    ),
                    save_model_config(),
                ),
    ...
    with st.expander("🦙 Ollama", expanded=st.session_state.active_model == "ollama"):
            ...
            ollama_enabled = st.toggle(
                ...
                on_change=lambda: (
                    setattr(
                        ...
                        "active_model",
                        "ollama"
                        if not st.session_state.active_model == "ollama"
                        else None,
                    ),
                    save_model_config(),
                ),
        ...
        if ollama_enabled:
            ...
            with col1:
                if st.button("检查连接", key="check_ollama_btn"):
            ...
            with col2:
                if st.button("刷新模型列表", key="refresh_models_btn"):
            ...
            st.markdown("---")
            st.markdown(
                "当前选择的模型: " + (selected_model if selected_model else "无")
            )
